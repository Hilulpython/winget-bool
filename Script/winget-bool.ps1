...
[string]$stdName = "WINGET_BOOL_EVAL|1|0"              # Default value if it doesnt exist
...
# Var Format:
# Name|True output|false output
# example: WINGET_BOOL_EVAL|1|0
#----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------#
...
#----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------#
[string[]]$VarContent = [Environment]::GetEnvironmentVariable($global:WG_Name, 'User') -split "|"

[Environment]::SetEnvironmentVariable($VarContent[0], $VarContent[2], 'User')
#----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------#
...
function Get-UpgradeVersion {
    [OutputType([Byte])]   # Output type indicator
    param (
        [string]$package,
        [bool]$show
    )
...
function Measure-Winget {
    [OutputType([bool])]   # Output type indicator
    param (
        [string[]]$WingetArgs
    )
...
function Format-MasterVar {
    ...
    [string]$MasterValue = [Environment]::GetEnvironmentVariable($global:WG_Name, 'User')
    [string[]]$Parts = $MasterValue -split "|"
    ...
    [string]$value = $name + "|" + $setTrue + "|" + $setFalse
    [Environment]::SetEnvironmentVariable($global:WG_Name, $value, 'User')
    Write-Host $value
}
#----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------#
...
#----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------#
if ($WingetArgs[0].ToLower() -match 'change') {
    if (2 -gt $WingetArgs.Length) {
        Write-Text "Kein Argumente gegeben"
        $global:outBool += $false
        return Format-Output
    }
    ...
    [Byte[]]$indexes = @()
    $CName = if (
        ($i = [Array]::IndexOf($WingetArgs, "-name")) -ge 0 -and
        ($i + 1 -lt $WingetArgs.Count) -and
        (-not $WingetArgs[$i + 1].StartsWith("-"))
    ) { $WingetArgs[$i + 1]} else { "" }

    $CTrue  = if (
        ($i = [Array]::IndexOf($WingetArgs, "-true" )) -ge 0 -and 
        ($i + 1 -lt $WingetArgs.Count) -and 
        (-not $WingetArgs[$i + 1].StartsWith("-"))
    ) { $WingetArgs[$i + 1] } else { "" }

    $CFalse = if (
        ($i = [Array]::IndexOf($WingetArgs, "-false")) -ge 0 -and 
        ($i + 1 -lt $WingetArgs.Count) -and 
        (-not $WingetArgs[$i + 1].StartsWith("-"))
    ) { $WingetArgs[$i + 1] } else { "" }

    Format-MasterVar $CName $CTrue $CFalse
